...
plt.hlines(M_wedge*air_after.a,np.rad2deg(w[-1]),np.rad2deg(beta_0),"k",label="wedge")
plt.ylabel("v [m/s]")
...
plt.hlines(M_wedge,np.rad2deg(w[-1]),np.rad2deg(beta_0),"k",label="wedge")
plt.ylabel(r"$M$")
plt.xlabel(r"$\omega$ [deg]")
plt.grid()
plt.legend(loc="center right")
...
for value_cone,value_wedge,ylabel in zip(thermoq_cone,thermoq_wedge,ylabels):
   ...
   plt.hlines(value_wedge,np.rad2deg(w[-1]),np.rad2deg(beta_0),"k",label="wedge")
   plt.ylabel(ylabel)
   plt.xlabel(r"$\omega$[deg]")
   plt.grid()
   plt.legend(loc="center right")
